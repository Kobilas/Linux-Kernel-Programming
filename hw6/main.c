...
/*
 * Matthew Kobilas 04/24/20
 * <linux/tty.h> for determining tty id of process
 * <linux/smpboot.h> for handling symmetric multiprocessing of some processes
 */
#include <linux/tty.h>
...
/*
 * Matthew Kobilas 04/24/20
 * function definition for printing thread information in pattern like ps -ef
 * prints all processes in formatted manner
 */
static void print_threadinfo(void) {
	...
	// prints ps-like column header along with name
	printk(KERN_INFO "Matthew Kobilas: %-*s %3s %5s %2s %-5s %-*s %8s %s\n",
			 k_uid_len, "UID", "PID", "PPID", "C", "STIME",
			 k_tty_len, "TTY", "TIME", "CMD");
	for_each_process(tmp_tsk) {
		if (uid_eq(tmp_tsk->cred->uid, GLOBAL_ROOT_UID)) {
			strncpy(uid, "root", k_uid_len);
		} else {
			snprintf(uid, k_uid_len, "%u", from_kuid(&init_user_ns, tmp_tsk->cred->uid));
		}
		...
		start_time_hr = time.tm_hour;
		start_time_mn = time.tm_min;
		time_hr = run_time / (NSEC_PER_SEC * 3600);
		time_mn = run_time / (NSEC_PER_SEC * 60) % 60;
		time_sc = run_time / NSEC_PER_SEC % 60;
		if (tmp_tsk->signal->tty) {
			strncpy(tty, tmp_tsk->signal->tty->name, k_tty_len);
			tty[k_tty_len] = '\0';
		} else {
			tty[0] = '?';
			tty[1] = '\0';
...
static int __ref kernel_init(void *unused)
{
	...
	/*
	 * Matthew Kobilas 04/24/20
	 * list process info similar to ps -ef
	 */
	print_threadinfo();
	// try to create custom threads as shown in homework example
	printk(KERN_INFO "Matthew Kobilas: m_k_t_do_something threads are about to be created.\n");
